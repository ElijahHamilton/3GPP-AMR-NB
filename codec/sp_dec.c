   ...
   if ( mode != MRDTX ) {
      ...
      for ( i = 1; i < DTX_HIST_SIZE; i++ ) {
         memcpy( &state->dtxDecoderState->lsf_hist[M * i], &state->
               dtxDecoderState->lsf_hist[0], sizeof( Word32 )*M );
      }
...
static void A_Refl( Word32 a[], Word32 refl[] )
{
   ...
   for ( i = M - 1; i >= 0; i-- ) {
      ...
      memcpy( aState, bState, sizeof( Word32 )*i );
   }
   return;
ExitRefl:
   memset( refl, 0, sizeof( Word32 )*M );
}
...
static void Build_CN_code( Word32 *seed, Word32 cod[] )
{
   ...
   memset( cod, 0, sizeof( Word32 )*L_SUBFR );
      ...
      , Word32 update )
{
   ...
   /* Copy mem[] to yy[] */
   memcpy( tmp, mem, sizeof( Word32 )*M );
   yy = tmp + M;
   yy_limit = yy + lg;
   a0 = a[0];
   ...
   memcpy( y, &tmp[M], sizeof( Word32 )*lg );

   /* Update of memory if update==1 */
   if ( update ) {
      memcpy( mem, &y[lg - M], sizeof( Word32 )*M );
   }
   return overflow;
}
      ...
      Word32 A_t[] )
{
   ...
   if ( ( st->dtxHangoverAdded != 0 ) & ( st->sid_frame != 0 ) ) {
      ...
      memcpy( &st->lsf_hist[ptr], &st->lsf_hist[st->lsf_hist_ptr], sizeof( Word32 )*M );
      ...
      memcpy( st->lsf_hist_mean, st->lsf_hist, sizeof( Word32 )*80 );
   ...
   if ( st->sid_frame != 0 ) {
       ...
       */
      memcpy( st->lsp_old, st->lsp, sizeof( Word32 )*M );
       ...
       */
         ...
         memcpy( lsfState->past_r_q, &past_rq_init[parm[0] * M], sizeof( Word32 )*M );
         D_plsf_3( lsfState, MRDTX, 0, &parm[1], st->lsp );

         /* reset for next speech frame */
         memset( lsfState->past_r_q, 0, sizeof( Word32 )*M );
         log_en_index = parm[4];
   ...
   memcpy( lsf_int_variab, lsf_int, sizeof( Word32 )*M );
   ...
   memcpy( lsfState->past_lsf_q, lsf_int, sizeof( Word32 )*M );
   ...
   memcpy( &A_t[0], acoeff, sizeof( Word32 )*MP1 );
   memcpy( &A_t[MP1], acoeff, sizeof( Word32 )*MP1 );
   memcpy( &A_t[MP1 <<1], acoeff, sizeof( Word32 )*MP1 );
   memcpy( &A_t[MP1 + MP1 + MP1], acoeff, sizeof( Word32 )*MP1 );
      ...
      , Word32 *lsp2_q )
{
   ...
   if ( bfi != 0 ) {
      ...
      memcpy( lsf2_q, lsf1_q, sizeof( Word32 )*M );
   ...
   memcpy( st->past_lsf_q, lsf2_q, sizeof( Word32 )*M );
...
static void decode_8i40_31bits( Word16 index[], Word32 cod[] )
{
   ...
   memset( cod, 0, sizeof( Word32 )*L_CODE );
      ...
      lsf_out[] )
{
   ...
   switch ( i_subfr ) {
      ...
      case 120:
         memcpy( lsf_out, lsf_new, sizeof( Word32 )*M );
         break;
   }
}
                    ...
                    Word32 pitch_fac, Word32 tmp_shift)
{
   ...
   if ( ( mode != MR122 ) & ( mode != MR102 ) & ( mode != MR74 ) & ( impNr < 2 )
      ...
      memcpy( inno_sav, inno, sizeof( Word32 )*L_SUBFR );
      memset( inno, 0, sizeof( Word32 )*L_SUBFR );
      ...
      frame[] )
{
   ...
   memcpy( &st->lsf_hist[st->lsf_hist_ptr], lsf, sizeof( Word32 )*M );
      ...
      enum RXFrameType frame_type, Word32 synth[], Word32 A_t[] )
{
   ...
   memcpy( prev_lsf, st->lsfState->past_lsf_q, sizeof( Word32 )*M );
   ...
   memcpy( st->lsp_old, lsp_new, sizeof( Word32 )*M );
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      /* MR122 */
         ...
         */
      memcpy( exc_enhanced, st->exc, sizeof( Word32 )*L_SUBFR );
      ...
      if ( pit_sharp > 16384 ) {
         ...
         overflow = Syn_filt( Az, exc_enhanced, &synth[i_subfr], L_SUBFR, st->mem_syn, 0
            );
      }
      else {
      ...
      if ( overflow ) {
         ...
         memcpy( st->mem_syn, &synth[i_subfr + 30], sizeof( Word32 )*M );
      }
         ...
         */
      memcpy( &st->old_exc[0], &st->old_exc[L_SUBFR], sizeof( Word32 )*( PIT_MAX + L_INTERPOL ) );
...
static int Decoder_amr_init( Decoder_amrState **state )
{
   ...
   *state = NULL;
   ...
   memset( s->Cb_gain_averState->cbGainHistory, 0, sizeof( Word32 )*L_CBGAINHIST );
