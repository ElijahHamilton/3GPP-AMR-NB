      ...
      *pred_init_i = 0;
      ...
      for ( j = 0; j < PAST_RQ_INIT_SIZE; j++ ) {
         ...
         if ( pred_init_err < min_pred_init_err ) {
            ...
            memcpy( lsf_r1, temp_r1, sizeof( Float32 )*M );
            memcpy( lsf_p, temp_p, sizeof( Float32 )*M );
            memcpy( past_rq, &past_rq_init[j * M], sizeof( Float32 )*M );
            *pred_init_i = j;
         }
      }
   }
      ...
      lsp_new[], Word16 **anap )
{
   ...
   memcpy( lsp_old, lsp_new, sizeof( Float32 )*M );
   memcpy( lsp_old_q, lsp_new_q, sizeof( Float32 )*M );
}
      ...
      Float32 error[] )
{
    ...
    */
   memcpy( ai_zero, Ap1, sizeof( Float32 )*MP1 );
   ...
   memcpy( exc, res2, sizeof( Float32 )*L_SUBFR );
      ...
      dn_sign[], Float32 cod[], Float32 h[], Float32 y[], Word16 *anap )
{
   ...
   memset( cod, 0, sizeof( Float32 )*L_CODE);
      ...
      cod[], Float32 h[], Float32 y[], Word32 sign_indx[], Word32 pos_indx[] )
{
   ...
   memset(cod, 0, L_CODE*sizeof(Float32));
      ...
      cod[], Float32 h[], Float32 y[], Word16 indx[] )
{
   ...
   memset( cod, 0, sizeof( Float32 )*L_CODE );
   memset( y, 0, sizeof( Float32 )*L_CODE );
      ...
      Word16 ana[], enum Mode *used_mode, Float32 synth[] )
{
   ...
   memcpy( st->new_speech, new_speech, sizeof( Float32 )*L_FRAME );
   ...
   if ( *used_mode == MRDTX ) {
      ...
      memset( st->old_exc, 0, sizeof( Float32 )*( PIT_MAX + L_INTERPOL ) );
      memset( st->mem_w0, 0, sizeof( Float32 )*M );
      memset( st->mem_err, 0, sizeof( Float32 )*M );
      memset( st->zero, 0, sizeof( Float32 )*L_SUBFR );
      memset( st->hvec, 0, sizeof( Float32 )*L_SUBFR );
      memset( st->lspSt->qSt->past_rq, 0, sizeof( Float32 )*M );
      memcpy( st->lspSt->lsp_old, lsp_new, sizeof( Float32 )*M );
      memcpy( st->lspSt->lsp_old_q, lsp_new, sizeof( Float32 )*M );
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      if ( ( evenSubfr != 0 ) && ( *used_mode == MR475 ) ) {
         memcpy( mem_syn_save, st->mem_syn, sizeof( Float32 )*M );
         memcpy( mem_w0_save, st->mem_w0, sizeof( Float32 )*M );
         memcpy( mem_err_save, st->mem_err, sizeof( Float32 )*M );
         sharp_save = st->sharp;
      }
      ...
      /* MR475 */
         ...
         if ( evenSubfr != 0 ) {
            memcpy( h1_sf0, st->h1, sizeof( Float32 )*L_SUBFR );
         }
      }

      /* copy the LP residual (res2 is modified in the CL LTP search) */
      memcpy( res2, res, sizeof( Float32 )*L_SUBFR );
      ...
      if ( *used_mode != MR475 ) {
         ...
         if ( evenSubfr != 0 ) {
            i_subfr_sf0 = i_subfr;
            memcpy( xn_sf0, xn, sizeof( Float32 )*L_SUBFR );
            memcpy( y2_sf0, y2, sizeof( Float32 )*L_SUBFR );
            memcpy( code_sf0, code, sizeof( Float32 )*L_SUBFR );
            T0_sf0 = T0;
            T0_frac_sf0 = T0_frac;
             ...
             */
            memcpy( st->mem_err, mem_err_save, sizeof( Float32 )*M );
...
void * Speech_Encode_Frame_init( int dtx )
{
   ...
   if (cod_amr_init( &s->cod_amr_state, dtx ) ) {
      ...
      prm, enum Mode *used_mode )
{
   ...
   for ( i = 0; i < 160; i++ ) {
      speech[i] = ( Word16 )( new_speech[i] & 0xfff8 )/2;
   }
